...
namespace Neo.Network.RPC.Tests
{
    ...
    public class UT_WalletAPI
    {
        ...
        public void TestSetup()
        {
            ...
            _address1 = Wallets.Helper.ToAddress(_sender, _client.protocolSettings.AddressVersion);
        ...
        public async Task TestClaimGas()
        {
            ...
            var tranaction = await _walletApi.ClaimGasAsync(_keyPair1.Export());
        ...
        public async Task TestTransfer()
        {
            ...
            var tranaction = await _walletApi.TransferAsync(NativeContract.GAS.Hash.ToString(), _keyPair1.Export(), UInt160.Zero.ToAddress(_client.protocolSettings.AddressVersion), 100);
            Assert.AreEqual(testScript.ToHexString(), tranaction.Script.Span.ToHexString());
        }
        ...
        public async Task TestTransferfromMultiSigAccount()
        {
            ...
            var tranaction = await _walletApi.TransferAsync(NativeContract.GAS.Hash, 1, new[] { _keyPair1.PublicKey }, new[] { _keyPair1 }, UInt160.Zero, NativeContract.GAS.Factor * 100);
            Assert.AreEqual(testScript.ToHexString(), tranaction.Script.Span.ToHexString());

            try
            {
                tranaction = await _walletApi.TransferAsync(NativeContract.GAS.Hash, 2, new[] { _keyPair1.PublicKey }, new[] { _keyPair1 }, UInt160.Zero, NativeContract.GAS.Factor * 100);
                Assert.Fail();
            }
            ...
            tranaction = await _walletApi.TransferAsync(NativeContract.GAS.Hash, 1, new[] { _keyPair1.PublicKey }, new[] { _keyPair1 }, UInt160.Zero, NativeContract.GAS.Factor * 100, string.Empty);
            Assert.AreEqual(testScript.ToHexString(), tranaction.Script.Span.ToHexString());
        }
