...
namespace Neo.Network.RPC.Tests
{
    ...
    public class UT_Nep17API
    {
        ...
        public async Task TestTransfer()
        {
            ...
            var result = await nep17API.CreateTransferTxAsync(client.protocolSettings, NativeContract.GAS.Hash, keyPair1, UInt160.Zero, new BigInteger(1_00000000));

            testScript = NativeContract.GAS.Hash.MakeScript("transfer", sender, UInt160.Zero, new BigInteger(1_00000000), string.Empty);
            UT_TransactionManager.MockInvokeScript(rpcClientMock, testScript, new ContractParameter());

            result = await nep17API.CreateTransferTxAsync(client.protocolSettings, NativeContract.GAS.Hash, keyPair1, UInt160.Zero, new BigInteger(1_00000000), string.Empty);
