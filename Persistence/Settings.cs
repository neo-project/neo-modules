...
namespace Neo.Plugins
{
	...
	//Height to begin real-time syncing and dumping on single files
        ...
        {
            this.BlockCacheSize = GetValueOrDefault(section.GetSection("BlockCacheSize"), 1000, p => uint.Parse(p));
            this.HeightBegin = GetValueOrDefault(section.GetSection("HeightToBegin"), 0u, p => uint.Parse(p));
