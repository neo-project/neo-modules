    ...
    public static class VRF
    {
        ...
        public static BigInteger GenerateNonce(byte[] prikey, byte[] data)
        {
            ...
            while (true)
            {
                ...
                if (ret_bn > BigInteger.Zero && ret_bn < Cryptography.ECC.ECCurve.Secp256r1.N)
                {
                    return ret_bn;
                }
        ...
        public static Cryptography.ECC.ECPoint ArbitraryStringToPoint(byte[] data)
        {
            byte[] v = new byte[] { 0x02 };
            v = v.Concat(data).ToArray();
            return Cryptography.ECC.ECPoint.FromBytes(v, Cryptography.ECC.ECCurve.Secp256r1);
        }
        ...
        public static Cryptography.ECC.ECPoint HashToTryAndIncrement(Cryptography.ECC.ECPoint pubkey, byte[] alpha)
        {
        ...
        public static BigInteger HashPoints(Cryptography.ECC.ECPoint[] points)
        {
        ...
        /// </returns>
        public static Tuple<Cryptography.ECC.ECPoint, BigInteger, BigInteger> DecodeProof(byte[] proof)
        {
            var Gamma = Cryptography.ECC.ECPoint.FromBytes(proof[..33], Cryptography.ECC.ECCurve.Secp256r1);
        ...
        public static byte[] GammaToHash(Cryptography.ECC.ECPoint gamma)
        {
        ...
        public static byte[] Prove(byte[] prikey, byte[] alpha)
        {
            ...
            var s = (k + (c * num_x)) % Cryptography.ECC.ECCurve.Secp256r1.N;
        ...
        /// "INVALID" </returns>
        public static byte[] Verify(Cryptography.ECC.ECPoint pubkey, byte[] proof, byte[] alpha)
        {
            ...
            // Step 5: hash points(...)
            var derived_c = HashPoints(new Cryptography.ECC.ECPoint[] { h_point, gamma_point, u_point, v_point });
        ...
        private static byte[] Bits2Octets(byte[] data, int qlen)
        {
            ...
            var z2 = z1 % Cryptography.ECC.ECCurve.Secp256r1.N;
