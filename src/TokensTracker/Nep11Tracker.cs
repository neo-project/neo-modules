...
namespace Neo.Plugins
{
    ...
    public class Nep11Tracker : Plugin, IPersistencePlugin
    {
        ...
        {
            _dbPath = GetConfiguration().GetSection("Nep11Path").Value ?? "Nep11BalanceData";
         ...
         JArray parentJArray, bool isSent)
        {
            ...
            var transferPairs = _db.FindRange<Nep11TransferKey, TokenTransfer>(
                prefix.Concat(startTimeBytes).ToArray(),
                prefix.Concat(endTimeBytes).ToArray());

            int resultCount = 0;
            var unsortList = new List<(Nep11TransferKey key, TokenTransfer value)>();
