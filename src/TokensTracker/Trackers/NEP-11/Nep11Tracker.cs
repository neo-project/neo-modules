...
namespace Neo.Plugins.Trackers.NEP_11
{
    class Nep11Tracker : TrackerBase
    {
        ...
        private void SaveDivisibleNFTBalance(TransferRecord record, DataCache snapshot)
        {
            ...
            {
                Log($"Fault: from[{record.from}] to[{record.to}] get {record.asset} token [{record.tokenId.ToBase64()}] balance fault", LogLevel.Warning);
                return;
            }
            ...
            {
                Log($"Fault: from[{record.from}] to[{record.to}] get {record.asset} token [{record.tokenId.ToBase64()}] balance not number", LogLevel.Warning);
                return;
            }
