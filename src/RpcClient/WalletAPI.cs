...
namespace Neo.Network.RPC
{
    ...
    public class WalletAPI
    {
        ...
        public async Task<Transaction> ClaimGasAsync(KeyPair keyPair, bool addAssert = true)
        {
            ...
            Transaction transaction = await _nep17Api.CreateTransferTxAsync(NativeContract.NEO.Hash, keyPair, toHash, balance, null, addAssert).ConfigureAwait(false);
        ...
        {
            Transaction transaction = await _nep17Api.CreateTransferTxAsync(scriptHash, from, to, amountInteger, data, addAssert).ConfigureAwait(false);
            await _rpcClient.SendRawTransactionAsync(transaction).ConfigureAwait(false);
            return transaction;
        }
        ...
        {
            Transaction transaction = await _nep17Api.CreateTransferTxAsync(scriptHash, m, pubKeys, keys, to, amountInteger, data, addAssert).ConfigureAwait(false);
            await _rpcClient.SendRawTransactionAsync(transaction).ConfigureAwait(false);
            return transaction;
        }
