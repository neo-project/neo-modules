...
namespace Neo.Plugins.Storage
{
    internal class Snapshot : ISnapshot
    {
        ...
        {
            batch.Delete(key ?? Array.Empty<byte>());
        }

        public void Put(byte[] key, byte[] value)
        {
            batch.Put(key ?? Array.Empty<byte>(), value);
        }
        ...
        {
            return db.Get(key ?? Array.Empty<byte>(), readOptions: options) != null;
        }

        public byte[] TryGet(byte[] key)
        {
            return db.Get(key ?? Array.Empty<byte>(), readOptions: options);
        }
