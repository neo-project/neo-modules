...
namespace Neo.Plugins.RestServer.Newtonsoft.Json
{
    public class WitnessJsonConverter : JsonConverter<Witness>
    {
        ...
        {
            var witnessObject = new JObject()
            {
                ["invocationScript"] = Convert.ToBase64String(value.InvocationScript.Span),
                ["verificationScript"] = Convert.ToBase64String(value.VerificationScript.Span),
                ["scriptHash"] = value.ScriptHash.ToString(),
            };
            witnessObject.WriteTo(writer);
        }
    }
}
