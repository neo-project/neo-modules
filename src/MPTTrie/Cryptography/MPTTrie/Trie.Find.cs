    ...
    partial class Trie
    {
        private ReadOnlySpan<byte> Seek(ref Node node, ReadOnlySpan<byte> path, out Node start)
        {
            switch (node.Type)
            {
                ...
                case NodeType.HashNode:
                    {
                        var newNode = _cache.Resolve(node.Hash) ?? throw new InvalidOperationException("Internal error, can't resolve hash when mpt seek");
                        node = newNode;
        ...
        private IEnumerable<(ReadOnlyMemory<byte> Key, ReadOnlyMemory<byte> Value)> Travers(Node node, byte[] path, byte[] from, int offset)
        {
            ...
            switch (node.Type)
            {
                ...
                case NodeType.HashNode:
                    {
                        var newNode = _cache.Resolve(node.Hash) ?? throw new InvalidOperationException("Internal error, can't resolve hash when mpt find");
                        node = newNode;
