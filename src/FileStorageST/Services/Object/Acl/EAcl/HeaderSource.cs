    ...
    public class HeaderSource : IHeaderSource
    {
        ...
        private List<XHeader> HeadersFromObject(FSObject.Object obj, Address address)
        {
            ...
            while (obj != null)
            {
                ...
                headers.Add(new XHeader
                {
                    Key = Filter.FilterObjectVersion,
                    Value = obj.Version.String()
                });
                headers.Add(new XHeader
                {
                    Key = Filter.FilterObjectPayloadHash,
                    Value = obj.PayloadChecksum.String()
                });
                headers.Add(new XHeader
                {
                    Key = Filter.FilterObjectType,
                    Value = obj.ObjectType.ToString()
                });
                headers.Add(new XHeader
                {
                    Key = Filter.FilterObjectHomomorphicHash,
                    Value = obj.PayloadHomomorphicHash.String()
                });
