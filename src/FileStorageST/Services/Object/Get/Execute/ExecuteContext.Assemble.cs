...
namespace Neo.FileStorage.Storage.Services.Object.Get.Execute
{
    public partial class ExecuteContext
    {
        ...
        {
            return Prm.Forwarder is not null;
        }
        ...
        private void Assemble()
        {
            ...
            {
                Log("GetExecutor", LogLevel.Warning, " could not init parent from child");
            }
        }

        private (ObjectID, List<ObjectID>) InitFromChild(ObjectID oid)
        {
            Log(nameof(Get.Execute), LogLevel.Debug, $"child id, {oid.String()}");
