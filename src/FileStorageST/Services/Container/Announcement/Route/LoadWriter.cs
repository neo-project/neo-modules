    ...
    public class LoadWriter : IWriter
    {
        ...
        public void Put(FSAnnouncement announcement)
        {
            ...
            foreach (var remoteInfo in value.Route)
            {
                ...
                if (!exists)
                {
                    ...
                    {
                        Log(nameof(LoadWriter), LogLevel.Warning, $"could not initilize writer or provider, error={e.Message}");
                        continue;
                    }
                ...
                {
                    Log(nameof(LoadWriter), LogLevel.Warning, $"could not put announcement, error={e.Message}");
                }
            }
        }
        ...
        public void Close()
        {
            foreach (var (_, writer) in mServers)
            {
                ...
                {
                    Log(nameof(LoadWriter), LogLevel.Warning, $"could not close writer, error={e.Message}");
                }
            }
        }
    }
