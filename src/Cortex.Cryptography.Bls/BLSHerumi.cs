...
namespace Cortex.Cryptography
{
    ...
    public class BLSHerumi : BLS
    {
        ...
        /// <inheritdoc />
        public override bool TryAggregateSignatures(ReadOnlySpan<byte> signatures, ReadOnlySpan<int> weights, Span<byte> destination, out int bytesWritten)
        {
            // This is independent of the keys set, although other parameters (type of curve, variant, scheme, etc) are relevant.
            ...
            if (signatures.Length / SignatureLength != weights.Length)
            {
                throw new ArgumentOutOfRangeException(nameof(signatures), signatures.Length, $"Signature data must be compatible with the weights length {weights.Length}.");
            }
            ...
            for (var index = 0; index < signatures.Length; index += SignatureLength)
            {
                ...
                int weight = weights[index / SignatureLength];
                ...
                else if (weight < 0)
                {
                    for (int i = 0; i < absWeight; i++)
                    {
                        Bls384Interop.SignatureSub(ref aggregateBlsSignature, ref blsSignature);
                    }
                }
                else continue;
            }
