...
namespace Neo.FileStorage.InnerRing.Services.Audit.Auditor
{
    public partial class Context
    {
        ...
        private void CheckStorageGroupPoR(int index, ObjectID oid)
        {
            ...
            {
                Utility.Log(nameof(Auditor), LogLevel.Debug, $"CheckStorageGroupPoR, get storage group failed, error={e}");
                return;
            }
            ...
            foreach (var member in members)
            {
                ...
                for (int j = 0; j < flat.Count; j++)
                {
                    ...
                    {
                        Utility.Log(nameof(Auditor), LogLevel.Debug, $"CheckStorageGroupPoR, get object header failed, error={e}");
                        continue;
                    }
                    UpdateHeader(header);
                    if (tzhash is null)
