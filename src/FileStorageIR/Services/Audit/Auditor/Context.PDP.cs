...
using static Neo.FileStorage.InnerRing.Services.Audit.Auditor.Helper;
    ...
    public partial class Context
    {
        ...
        private void CollectHashes(GamePair pair)
        {
            ...
            foreach (var range in pair.Range1)
            {
                ...
                {
                    Utility.Log(nameof(Auditor), LogLevel.Debug, $"node1 rang hash failed. range={range}. error={e}");
                    return;
                }
            }
            ...
            foreach (var range in pair.Range2)
            {
                ...
                {
                    Utility.Log(nameof(Auditor), LogLevel.Debug, $"node2 rang hash failed. range={range}. error={e}");
                    return;
                }
            }
        }
