...
namespace Neo.Plugins
{
    ...
    {
        private readonly Dictionary<uint, JArray> _bsCache = new Dictionary<uint, JArray>();
        ...
        private void OnPersistStorage(uint network, DataCache snapshot)
        {
            ...
            if (blockIndex >= Settings.Default.HeightToBegin)
            {
                ...
                JObject bsItem = new JObject();
                bsItem["block"] = blockIndex;
                bsItem["size"] = array.Count;
                bsItem["storage"] = array;
                if (!_bsCache.TryGetValue(network, out JArray cache))
                {
                    cache = new JArray();
                }
                cache.Add(bsItem);
                _bsCache[network] = cache;
            }
        }
        ...
        {
            if (!_bsCache.TryGetValue(network, out JArray cache)) return;
